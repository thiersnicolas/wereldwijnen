  	...
  	<div id="hoofding">
  		...
  		<ul id="vlaggen" class="nav clearFix">
  			<c:forEach var="land" items="${landen}">
  				...
  				<li class="nav item vlag"><a class="vlag-link" href="${indexURL}"><img class="nav-link" src="<c:url value='/images/${land.id}.png'/>" alt="${land.naam}"/></a></li>
  			</c:forEach>
  		</ul>
  	</div>
  	...
  	<c:if test="${not empty wijnen}">
  		...
  		<ul>
  			<c:forEach var="wijn" items="${wijnen}">
  				...
  				<li class="wijnen"><a href="${indexWijnURL}">${wijn.jaar} <c:forEach var='index' begin='1' end='${wijn.beoordeling}'>&#9733;</c:forEach></a></li>
  			</c:forEach>
  		</ul>
  	</c:if>
  	</div>
